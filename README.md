...
| 경매장 조회 | 아이템 경매장 검색 (필터, 정렬, 페이지네이션) | ✅ |
...
#### GET `/api/v1/auctions/options`
경매장 검색 필터 UI 구성에 필요한 메타데이터 조회. 1시간 캐싱.
카테고리 코드, 직업 목록, 아이템 등급/티어, 각인/전투특성 옵션 포함.

#### POST `/api/v1/auctions/items`
경매장 아이템 검색. 1분 캐싱 (실시간 가격 반영).

**왜 캐시 키를 검색 조건 조합으로 설정했는가?**
경매장은 카테고리·이름·등급·티어·정렬·페이지 조합마다 다른 결과를 반환합니다. 동일 조건 반복 요청을 캐시에서 처리하되, 조건이 다르면 항상 새 API 호출을 하도록 조건 필드 조합을 캐시 키로 사용했습니다.
